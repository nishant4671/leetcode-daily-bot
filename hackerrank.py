...
2. Read all input directly from standard input (e.g., using sys.stdin.read().split() or input()).
3. Print all output directly to standard output (using print()).
...
for attempt in range(20):
    ...
    if status not in ["Working", "Queued", "Compiling"]:
        if status == "Accepted":
            send_telegram(
                f"✅ *HackerRank Solved!*\n\n"
